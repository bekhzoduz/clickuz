...
if ($_POST['action'] == 1 and $_SERVER['REMOTE_ADDR'] == "55.10.022.300")
{
    ...
    if (strlen($row['id']) > 0)
    {
        if ($error == 0)
        {
            ...
            $db->query("UPDATE users SET balance = $balance WHERE id = $merchant_id");
            $db->query("UPDATE clickuz SET situation = '1' WHERE trans_id = $trans_id");
